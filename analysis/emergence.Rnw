...
\section*{Estimation}
Our classic methods, whether frequentist or Bayesian, for estimating parameters depend upon being able
to calculate the likelihood of the data. However, while the spatial
model looks similar to the logistic regression models, the full data
likelihood turns out to be intractable. The likelihood depends not
just on the observed patterns of racial segregation, but every single
possible pattern of racial segregation.

Let $\mathbf{z}$ be a possible pattern of racial segregation
and $\zeta$ be the set of all possible patterns.

\begin{align}
  Z = \sum_{\mathbf{z} \in \zeta} \exp\Big({\sum_i^N(\beta_0_z_i + ... + \beta_dx_dz_i) + \sum_i\sum_{j \in \operatorname{N}(i)}\rho z_iz_j}\Big)
\end{align}
and the full data log likelihood of an observed pattern of racial
segregation, $\mathbf{y}$, is 

\begin{align}
  \log\Pr(\mathbf{y}) = - \log Z + &\sum_i^M(\beta_0_y_i + \beta_1x_{0i}y_i +
  ... + \beta_dx_dy_i) + \sum_i\sum_{j \in \operatorname{N}(i)}\rho
  y_i,y_j
\end{align}
where
\clearpage
...
\begin{align}
  \log\frac{\Pr(\mathbf{y})} = \sum_i^N\beta_0y_i + \beta_1x_{0i}y_i +
  \beta_2x_{1i}y_i + ... + \rho\sum_{j \in \operatorname{N}(i)}y_j
\end{align}



Without the last, spatial
component, this would be normal problem of logistic
regression. However, the spatial auto-correlation turns this into a
problem that we can not estimate using classic maximum likelihood or
Bayesian approaches.

This is easier to see with a simpler model, where the only site
specific factor for whether a black person lives at the $i$th site is
the race of the neigbors (Equation \ref{equation:autolog}).
\begin{align}
\log\frac{\Pr(y_i = 1 \mid y_j : j \neq i)}{\Pr(y_i = 0 \mid y_j : j \neq i)} = b_0 + \rho\sum_{j \in \operatorname{N}(i)}y_j
