...
blockgroups\footnote{https://www.census.gov/geo/maps-data/data/tiger.html,
  PySAL}. There are \Sexpr{n_blockgroups} block groups in the data.
...
\begin{align}
  \epsilon_{i} = b_0 + b_1\operatorname{B_1}(y_{i}) 
...
results, and repeating 1000 times. Using the a percentile bootstrap,
we to estimate the confidence intervals for the parameters.

In Table~\ref{tab:parameters}, we have the parameters for racial
preferences. The values may seem very small, but this is due to the
...
If a racial or ethnic preference parameter is positive, then blocks of
that type should tend to be neighbors, all else equal. If a preference
parameter is negative, then blocks of that type should tend to not be
...
is positives, suggesting that white and Hispanic groups should tend to
neighbor, all else being equal. 

Only two parameters have 95\% confidence intervals that do not cover
0. 
