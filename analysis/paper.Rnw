...
s  prettyNum(x, big.mark=",")
})
@ 

Sociologists know that large scale patterns can emerge from local
interactions of individuals. but this is largely a theoretical
insight. From thought experiments and simulations, we can see how
racial segregation, flocking behavior, or public order on the street
could emerge from individuals making simple, local adjustments to one
another. Unfortunately, models of emergent order have been hard to
test empirically.

First, in many social settings, we cannot tell whether Alice jumped
off the bridge because that's what her friend Bob did; or because
Alice or Bob are bridge-jumping enthusiasts and that's why they are
friends; or because they were taking a friendly walk together when the
bridge was attacked by Godzilla. If we do not have an adequate model
of why two people are interacting, we cannot distinguish the effects
of that interaction from unobserved homophily or common
forces.\footnote{shalizi, homophily}

Second, models with interdependent units are computationally harder to
