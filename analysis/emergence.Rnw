...
\section{Racial Segregation, an Interdependent Approach}
In Schelling model, a household is not discontent until it surrounded
by a majority of neighbors that are of a different race. This type of
nonlinearity 

Some of the features of Schelling's model 



Even with a great deal more data and
computing power, we will still have to make some stricter assumptions 

This ``Schelling measure'' requires stricter
assumptions and a lot of data
