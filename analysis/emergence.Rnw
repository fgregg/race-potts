...
names(tract_params) <- c('City', 'N', 'Average Neighbors',
...
<<blockgroups-intercept, results='asis', cache=FALSE>>=
tract_params <- read.csv('params_blockgroups_2010.csv', header=FALSE)
names(tract_params) <- c('City', 'N', 'Average Neighbors',
                         'Int., Black', 'Median Housing Cost, Black', 
                         'B-H', 'B-W',
                         'Int., Hispanic', 'Median Housing Cost, Hispanic', 
...
no_x <- tract_params[, c('City',
                         ...
                         'H-B', 'H-W')]
xtable(no_x[order(no_x$"B-W"),],
       caption="2010 Block Groups, No Housing Costs")
@
