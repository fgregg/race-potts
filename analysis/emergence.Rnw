...
phenomena, but they all share a common hurdle. For the small and
moderate sized network data typical of social science, we can place
very little trust in our estimates.  Our techniques for estimating
these types of models are either computationally intractable or
statistically inefficient. If our data is small enough to use a
statistically efficient technique, the meager size of the data will
lead to large sampling error. If we have more data, we will have use a
much less efficient method.

The simplistic escape from this dilemma is more data. Since
sociologists are increasingly able to access very large network data
sets, we can benefit from techniques that we previously would have
ignored.

In this paper, we discuss one such method using as an example the
classic problem of racial segregation. We will produce new measures
of city level segregation called the ``Schelling index'', which
measures how much local, racist housing preferences are necessary to
produce observed city-level racial segregation. This measures and the
model we use to derive it opens up new insights and approaches in the
study of housing segregation.
