...
In order to compare the power of these two explanations, we might
want to see which factor explained more of observed residential racial
segregation in American cities. For the moment, simplifying to a
black and white world, we would write down a model that
looked something like Equation \ref

\begin{align}
  \log\frac{\Pr(y_i = 1)}{\Pr(y_i = 0)} = \beta_0 + \beta_1x_{0i} +
  \beta_2x_{1i} + ... + \rho_1\sum_{j \in \operatorname{N}(i)}y_j
\end{align}
where $y_i$ is the race of the person living at the $i$th site and
$y_i=1$ if a black person lives the $i$th site, $0$ otherwise. The
$x$'s are attributes of the $i$th site and together with the $\beta$'s
affect the log odds of a black person living at the $ith$ site,
unconditional of neighbors. $\rho$ would be effect of the race of the
neighbors on the the log odds; $\operatorname{N}(i)$ is a function
that returns the indices of the neighbors of the $i$th location.

If we could estimate this equation, we would be in a strong position
to compare the importance of the mechanisms for racial
segregation. If $\rho$ is a persistent and consequential component of
the model, that would be strong evidence for racial segregation as a
continued, emergent phenomena. Of course, the standard difficulties of
model identification still hold.

Without the last, autospatial component, this would be normal pHowever, this model is not estimable using maximum likelihood or
Bayesian approaches. 




If we could estimate this for a snapshot of 

Without the last term,  estimating the $\beta$'s is a standard problem of logistic
regression,With the last term, this a 
Without 
